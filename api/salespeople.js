  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      try {
        ...
        if (!createdUser?.id) return json(res, 502, { error: "Unable to create authentication user" });

        const salesperson = await createSalespersonRow({
          ...payload,
          auth_user_id: createdUser.id,
        });
        if (!salesperson?.id) return json(res, 502, { error: "Unable to create salesperson record" });
    ...
    if (req.method === "PATCH") {
      ...
      if (looksLikeUuid(authUserId)) {
        ...
        if (!authUserId) return json(res, 502, { error: "Unable to create authentication user" });
      }
