...
const T = {
  en: {
    ...
    fiscalSequenceExpiration:"Sequence Expiration", vendor:"Vendor", notConfigured:"Not configured",
  ...
  es: {
    ...
    fiscalSequenceExpiration:"Vencimiento de secuencia", vendor:"Vendedor", notConfigured:"No configurado",
...
/* ════════════════════════════════════════════════════════════
   PDF — FISCAL (DGII)
════════════════════════════════════════════════════════════ */
const makeFiscalPDF=(doc,company,fmt,t,settings={})=>{
  const snap=doc.client_snapshot||{};
  const fiscalNumber=doc.ncf||doc.order_number||"";
  const expiration=settings.fiscal_sequence_expiration_date||"";
  ...
  const ncfBlock=fiscalNumber?`
    <div style="margin-top:10px;background:#f0eaf8;border:2px solid #4a1a8a;border-radius:8px;padding:10px 16px;display:inline-block;min-width:260px">
      ...
      <div style="font-size:18px;font-weight:900;color:#4a1a8a;letter-spacing:.04em;word-break:break-all">${fiscalNumber}</div>
...
export default function App() {
  ...
  const [settings,setSettingsState]=useState({lang:"es",currencySymbol:"RD$",currencyPosition:"before",fiscal_sequence_expiration_date:"",taxes:[{id:"t1",name:"ITBIS",rate:18}]});
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
      ...
      {modal==="invoice"&&previewDoc&&<Sheet title={`${(previewDoc.type||"").toUpperCase()} · ${previewDoc.order_number}`} onClose={()=>{setModal(null);setPreviewDoc(null);}} wide>
        <InvoicePreview t={t} settings={settings} doc={previewDoc} fmt={mkFmt(previewDoc.currency_symbol||settings.currencySymbol,previewDoc.currency_position||settings.currencyPosition)} company={previewDoc._company||companies.find(c=>c.id===previewDoc.company_id)}/>
      </Sheet>}
  ...
  return(
    <div style={{flex:1,overflowY:"auto",padding:"14px"}}>
      <div style={{display:"flex",flexDirection:"column",gap:"12px",maxWidth:"520px",margin:"0 auto"}}>
        ...
        <Card>
          <div style={{fontSize:"11px",fontWeight:"700",color:C.muted,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:"12px"}}>{t.fiscalSequenceExpiration}</div>
          <Inp type="date" value={s.fiscal_sequence_expiration_date||""} onChange={v=>upd("fiscal_sequence_expiration_date",v)} sm/>
        </Card>
        <Card>
...
/* ════════════════════════════════════════════════════════════
   INVOICE PREVIEW
════════════════════════════════════════════════════════════ */
function InvoicePreview({t,settings,doc,fmt,company}){
  const snap=doc.client_snapshot||{};
  const isFiscal=doc.is_fiscal&&company?.tax_enabled;
  const st=STATUS[doc.status]||STATUS.ordered;

  const printDoc=()=>{
    const html=isFiscal?makeFiscalPDF(doc,company,fmt,t,settings):makeCommercialPDF(doc,company,fmt,t);
