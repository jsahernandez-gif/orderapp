...
const T = {
  en: {
    ...
    itemsSummary:"Items Summary", activeFilters:"Filters", importHelpProducts:"Columns: name, reference_code, category, price, unit, image_url",
    importHelpClients:"Columns: name, email, phone, address, rnc, image_url", filterClient:"Client", filterStatus:"Status",
  ...
  es: {
    ...
    itemsSummary:"Resumen de Artículos", activeFilters:"Filtros", importHelpProducts:"Columnas: name, reference_code, category, price, unit, image_url",
    importHelpClients:"Columnas: name, email, phone, address, rnc, image_url", filterClient:"Cliente", filterStatus:"Estado",
    waMsg: (doc, items, total, due) =>
...
const PRODUCT_TEMPLATE_ROWS = [
  ["name","reference_code","category","price","unit","image_url"],
  ["required","optional","optional","required","optional","optional"],
  ["Water Bottle","WB-100","Beverages","125.50","unit","https://example.com/image.jpg"],
];
const CLIENT_TEMPLATE_ROWS = [
  ["name","email","phone","address","rnc","image_url"],
  ["required","optional","optional","optional","optional","optional"],
  ["Cliente Demo","cliente@example.com","(809) 555-1212","Santo Domingo","101123456","https://example.com/client.jpg"],
];
...
function MainApp({user,salesperson,t,fmt,settings,saveSettings,showToast,askConfirm,onLogout,setLightbox}){
  ...
  const saveOpenOrder=async()=>{
    ...
    const orderNum=existing?.order_number||await getNextOrderNum();
    const payload={
  ...
  const downloadTemplate=(entity)=>{
    ...
    downloadTextFile(`${entity}-template.csv`,toCsv(rows));
    showToast(t.templateDownloaded||"Template downloaded ✓");
  };
  const exportProductsCsv=()=>{
    const rows=[
      ["id","name","reference_code","category","price","unit","image_url"],
      ...products.map(p=>[p.id||"",p.name||"",p.reference_code||"",p.category||"",safeNum(p.price),p.unit||"",p.image_url||""]),
    ];
    downloadTextFile(`products-${todayISO()}.csv`,toCsv(rows));
  };
  const exportClientsCsv=()=>{
    const rows=[
      ["id","name","email","phone","address","rnc","image_url"],
      ...clients.map(c=>[c.id||"",c.name||"",c.email||"",c.phone||"",c.address||"",c.rnc||"",c.image_url||""]),
    ];
    downloadTextFile(`clients-${todayISO()}.csv`,toCsv(rows));
  };
  ...
  const importProductsFile=async(file)=>{
    ...
    for(const row of rows){
      ...
      if(!name){errors.push(`${t.importRowError} ${row.rowNumber}: name is required.`);continue;}
      if(values.price===""||!Number.isFinite(Number(values.price))){errors.push(`${t.importRowError} ${row.rowNumber}: price must be numeric.`);continue;}
      if(referenceCode&&/[^a-z0-9_-]/i.test(referenceCode)){errors.push(`${t.importRowError} ${row.rowNumber}: reference_code must be alphanumeric.`);continue;}
      try{
  ...
  const importClientsFile=async(file)=>{
    ...
    for(const row of rows){
      ...
      if(!name){errors.push(`${t.importRowError} ${row.rowNumber}: name is required.`);continue;}
      if(email&&!looksLikeEmail(email)){errors.push(`${t.importRowError} ${row.rowNumber}: email is invalid.`);continue;}
      try{
        ...
        {cartItems.length>0?cartItems.map(i=>(
          <div key={i.id} style={{background:C.bg,borderRadius:"10px",padding:"10px",display:"flex",flexDirection:"column",gap:"8px"}}>
            <div style={{display:"flex",justifyContent:"space-between",gap:"10px"}}>
              ...
              <div style={{fontWeight:"700",fontSize:"13px"}}>{fmt(i.line_total)}</div>
            </div>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:"12px",color:C.muted}}>
              <span>{fmt(i.price)} × {i.qty}</span>
              <span>{t.gross}: {fmt(i.gross_total)}</span>
            </div>
