...
const T = {
  en: {
    appName: "OrderApp",
    login:"Sign In", logout:"Sign Out", email:"Email", password:"Password", loginErr:"Invalid credentials.", loading:"Loading…",
  ...
  es: {
    appName: "OrderApp",
    login:"Iniciar Sesión", logout:"Cerrar Sesión", email:"Correo", password:"Contraseña", loginErr:"Credenciales incorrectas.", loading:"Cargando…",
...
const fmtPhone  = (raw) => { const d=raw.replace(/\D/g,"").slice(0,10); if(d.length<=3) return d.length?`(${d}`:""; if(d.length<=6) return `(${d.slice(0,3)}) ${d.slice(3)}`; return `(${d.slice(0,3)}) ${d.slice(3,6)}-${d.slice(6)}`; };
...
function MainApp({user,salesperson,t,fmt,settings,saveSettings,showToast,askConfirm,onLogout,setLightbox}){
  ...
  const loadAll=useCallback(async()=>{
    ...
    if(Array.isArray(co)){
      setCompanies(co);
      // ── CRITICAL FIX: only set default company if user hasn't picked one yet
      if(!userPickedCompany.current&&co.length){
        _setSelCompany(co[0]);
        setUseTax(co[0].tax_enabled);
      }
      // Load NCF configs
      const cfgs={};
      for(const c of co){
  ...
  const finalizeOpenOrder=async(order,type,company)=>{
    ...
    const updated=await sb.q("orders").update(payload,{eq:{id:order.id}});
    const finalDoc={...order,...payload,_company:co};
    setOrders(os=>os.map(o=>o.id===order.id?{...o,...payload}:o));
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
      ...
      {/* ── Modals ── */}
      {modal==="newOrder"&&(
        <Sheet title={editingId?`${t.edit} ${t.orders}`:t.newOrder} onClose={()=>{setModal(null);clearOrder();}} wide
...
function OrderBuilder({t,fmt,settings,clients,products,categories,team,companies,ncfConfigs,cart,setQty,cartItems,subtotal,discount,setDiscount,total,taxLines,discountAmt,taxTotal,cartCount,selClient,setSelClient,selCompany,setSelCompany,activeCat,setActiveCat,searchQ,setSearchQ,notes,setNotes,selSp,setSelSp,useTax,setUseTax,payType,setPayType,creditDays,setCreditDays,finalizeOrder,saveOpenOrder,clearOrder,editingId,salesperson,setModal,orderMode,setOrderMode}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"13px"}}>
        ...
        <div>
          ...
          <div style={{display:"flex",gap:"7px"}}>
            {companies.filter(c=>c.is_active).map(co=>(
              <button key={co.id}
                onClick={()=>{
                  setSelCompany(co);   // user explicit pick — won't be overridden
          ...
          {selCompany?.tax_enabled&&nextNCF&&orderMode==="invoice"&&(
            <div style={{marginTop:"7px",background:C.fiscalSoft,borderRadius:"8px",padding:"7px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      {/* Action buttons */}
      {orderMode==="open"?(
        <Btn onClick={saveOpenOrder} disabled={!selClient||cartItems.length===0} variant="open" full size="lg">
          📂 {t.createOpenOrder}
        </Btn>
      ):(
...
function PersonForm({t,person,onSave}){
  const [f,setF]=useState({name:"",role:"",email:"",phone:"",is_admin:false,permissions:{...DEFAULT_PERMS},...person,permissions:{...DEFAULT_PERMS,...(person?.permissions||{})}});
  const u=k=>v=>setF(p=>({...p,[k]:v}));
